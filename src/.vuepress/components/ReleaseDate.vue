...
export default {
    ...
    async mounted() {
        ...
        try {
            ...
            this.$data.nightlyPublishRelative = this.$moment(data.published_at).fromNow();
            this.$data.nightlyPublishExact = this.$moment(data.published_at).format("dddd, MMMM Do YYYY [at] HH:mm");
        } catch (e) {
            console.error(e);
        }
    },
